...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, any>((props, ref) => {
  ...
  return (
    <div ref={ref} className={cn("grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-[color:hsl(var(--border)_/_0.5)] bg-background px-2.5 py-1.5 text-xs shadow-xl", className)}>
      ...
      <div className="grid gap-1.5">
        {payload.map((p: any, index: number) => {
          ...
          return (
            <div key={p.dataKey ?? index} className={cn("flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-muted-foreground", indicator === "dot" && "items-center")}>
              {formatter && p?.value !== undefined && p.name ? (
                formatter(p.value, p.name, p, index, p.payload)
              ) : (
                <>
                  {itemConfig?.icon ? (
                    <itemConfig.icon />
                  ...
                  <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                    <div className="grid gap-1.5">
                      {nestLabel ? tooltipLabel : null}
                      <span className="text-muted-foreground">{itemConfig?.label || p.name}</span>
                    </div>
                    {p.value != null && (
                      <span className="font-mono font-medium tabular-nums text-foreground">{(p.value as any).toLocaleString?.() ?? String(p.value)}</span>
                    )}
                  </div>
                </>
              )}
